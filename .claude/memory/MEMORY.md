...
- [visual_mistakes.md](visual_mistakes.md) — append-only figure/demo layout mistake KB (VM1-VM35); the visual-audit agent (../agents/visual-audit.md) re-reads it on EVERY audit — run that agent on every figure/demo before showing the user. Agent now has C0 spec-fidelity · C11 every-text-BOLD · C12 white-space measurement (added 2026-07-27)
- [feedback_never_override_user_spec.md](feedback_never_override_user_spec.md) — ⛔ **COSTLY 2026-07-27**: NEVER drop/reformat/substitute what the user explicitly spec'd (metrics, named rivals, plot TYPE, panel count, cited reference) to satisfy readability/cleanliness; when spec conflicts with a constraint, STOP and ask a spec-PRESERVING question — never a menu that abandons the ask. Added audit check C0 SPEC-FIDELITY + VM34
