...
Decode substitution (documented): PyAV is absent in the demo venv (venv_walkindia) — frames come
...
def scene_cards(cv, sink, dcfg, labels, n_clips, scenes, verdict_rows=None):
    ...
    if verdict_rows is None:
        if "W" in scenes:
            ...
            d.text((cv.W // 2, 320), "real WalkIndia street clips — the SECOND HALF of each clip is hidden from both models",
            ...
            d.text((cv.W // 2, 250), "   vs   ".join(labels), font=_font(26), fill=_ACC, anchor="mm")
            d.text((cv.W // 2, 320), "V-JEPA 2.1 ViT-g (1B) encoder + its own predictor  ·  real WalkIndia street clips",
                   font=_font(16), fill=_SUB, anchor="mm")
