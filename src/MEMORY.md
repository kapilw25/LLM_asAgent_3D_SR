...
- **`configs/legacy2/surgery_2stage_noDI_encoder.yaml`**: `vjepa_2_1_surgical_noDI`. Stage 1 unfreeze 0.25 → `L:1.00`; Stage 2 unfreeze 0.50 → `L:0.30, A:0.70`. 50/50 budget.
- **`configs/legacy2/surgery_2stage_loud_agent.yaml`**: `vjepa_2_1_surgical_loud_agent` (iter10 v15b). Stage 2 mix `L:0.15, A:0.85`.
- **`configs/train/surgery_3stage_DI_encoder.yaml`**: `vjepa_2_1_surgical_3stage_DI`. `interaction_mining.enabled: true`. 0.40/0.30/0.30 budget. Stage 3 unfreeze stays 0.50 (NOT prior 0.75 BWT=−0.33 over-destabilization). Mix `L:0.15, A:0.15, I:0.70`.
...
| Modules | `probe_action.py` + `probe_motion_cos.py` + `probe_future_mse.py` (+ utils `action_labels.py`, `frozen_features.py`); orchestrated by `scripts/run_eval.sh` | `m12_action_labels.py` + `m13_probe_train.py` + `m13b_factor_probe_train.py` (NOT YET BUILT) |
...
- LOW: `iter/iter13_motion_probe_eval/runbook.md` — terminal commands for `scripts/run_eval.sh`. Iter12 runbook still useful for m13/m13b pattern (`iter/iter12_multitask_LOSS/runbook.md`); iter11 runbook at `iter/iter11_epoch15/runbook.md`.
