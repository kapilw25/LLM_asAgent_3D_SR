...
# (200-clip smoke moved to Box-3 — it needs the et-heads, which only exist after Box-3's --etheads-only gen)
...
```bash
# 2. the et-heads gen corpus (Stage 8c trains on eval_10k) , # 3. FULL folder, NO --ext (local==HF; partial hides stale HF files)
python -u src/utils/hf_outputs.py download-data data/subset_10k_local 2>&1 | tee logs/download_data_subset_10k_local_$(date +%Y%m%d_%H%M%S).log && \
python -u src/utils/hf_outputs.py download-data data/eval_10k_local 2>&1 | tee logs/download_data_eval_10k_local_$(date +%Y%m%d_%H%M%S).log && \                
...
2>&1 | tee logs/m12f_sanity_$(date +%Y%m%d_%H%M%S).log

# encoder-temporal aot/tov/pace heads on eval_10k = cross-set reuse SOURCE, 4-wide (~5 h). MUST precede the retest exports below.
python -u scripts/iter18_poc_ngpu.py --mode POC --gpus 4 \
--etheads-only --cache 2 --skip-arms $SKIP \
2>&1 | tee logs/regen_etheads_ngpu_$(date +%Y%m%d_%H%M%S).log

# 200-clip smoke → subset_10k_smoke: cheap end-to-end check of the cross-set head-reuse path before the 8-12h retest
...
| taxonomy-head regen (17 enc, sequential) | Box-2 · 1× RTX 6000 | ~15:00 |
| encoder-temporal head regen (aot/tov/pace, eval_10k, 4-wide) | Box-3 · 4× RTX 6000 | ~5:00 |
| 200-clip cross-set smoke | Box-3 · 4× RTX 6000 | ~0:10 |
