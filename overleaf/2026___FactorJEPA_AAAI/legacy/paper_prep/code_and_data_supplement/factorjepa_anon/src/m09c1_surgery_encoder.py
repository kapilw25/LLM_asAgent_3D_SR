...
def build_model(cfg: dict, device: torch.device) -> dict:
    ...
    # Verified at HF endpoint anon-org/factorjepa-pretrain-vjepa21-vitg-5ep.
...
def train(cfg: dict, args):
    ...
    # — at POC: data/eval_10k_local/eval_10k_val_split.json (125 stratified POC val clips); at
    ...
    try:
        for stage_idx, stage_cfg in enumerate(stages):
            ...
            pbar.close()
            if kill_state["triggered"]:
                # Stage ckpt = resume/rollback anchor → full_resume_anchor: poc/full=True so
                # optimizer+scheduler+scaler restore; sanity=False → ~7GB student-only smoke ckpt.
...
def main():
    ...
    parser.add_argument("--init-from-ckpt", type=str, required=True,
                        help="iter14 REQUIRED: HF model-repo URI for prior pretrain student_encoder.pt. "
                             ...
                             "Example: hf://anon-org/factorjepa-pretrain-vjepa21-vitg-5ep/m09a_ckpt_best.pt. "
