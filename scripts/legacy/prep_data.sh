...
# Pipeline: m04(tags) → m04d(motion) — TAGS + MOTION ONLY
...
# ── Embedding: handled by run_embed.sh (standalone, reusable across all chapters)
log "Tags + motion complete. Next steps:"
log "  ./scripts/run_embed.sh --${MODE} --local-data ${LOCAL_DATA}"
log "  ./scripts/run_eval.sh --${MODE}"
